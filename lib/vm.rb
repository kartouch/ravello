...
class Vm
  attr_accessor :name, :description, :creation_time, :memory_size, :num_cpus, :platform, :os,
  ...
  def initialize(vm)
    obj = [:name, :description, :creation_time, :memory_size, :num_cpus, :platform, :os,
            :supports_cloud_init, :user_data, :requires_key_pair, :key_pair_id, :hostnames, :application_id,
            :legacy_mode , :loading_status, :loading_percentage, :base_vm_id, :bios_uuid, :cpu_ids, :boot_order,
            :rtc, :display_device, :vm_order_group_id, :stop_time_out]
    obj.each{|x| instance_variable_set("@#{x.to_s}", vm[x.to_s.camel_case_lower]) unless x.nil?}
